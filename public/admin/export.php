...
$exam = $db->query("SELECT * FROM exams WHERE id = '$id'")->fetch_assoc();
...
if (isset($_GET['class'])) {
    ...
    if ($db->query("SELECT * FROM exams_class WHERE exam_id = '$id' AND class = '$class'")->fetch_assoc() && $db->query("SELECT * FROM teachers_class WHERE teacher_id = '$teacher[id]' AND class = '$class'")->fetch_assoc()) {
        if (isset($_GET['type']) && $_GET['type'] == 'grade') {
            ...
            while ($question = $questions->fetch_assoc()) array_push($line, $question['question_text']);
            $fp = fopen('php://output', 'wb');
            fputcsv($fp, $line, ',');
