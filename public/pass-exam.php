...
if (!($exam = $results->fetch_assoc())) exit(header('location: /'));
...
if (!($session = $results->fetch_assoc())) {
    $start_at = time();
    $end_at = strtotime("+$exam_duration minutes", time());
    $db->execute_query("INSERT INTO SESSIONS(student_id, exam_id, start_at, end_at) VALUES ($student_id, '$exam_id', '$start_at', '$end_at')");
    $session = $db->query("SELECT * FROM SESSIONS WHERE student_id = $student_id AND exam_id = '$exam_id'");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    while ($question = $questions->fetch_assoc()) {
        $score = 0;
        $answer = $db->real_escape_string($_POST["answer_" . $question['id']]);
        if ($question['type'] == 'short_answer') $score = -1;
            ...
            $answer = $db->real_escape_string(json_encode($answer));
        } else if (strtolower($_POST["answer_" . $question['id']]) == strtolower($question['correct_answer'])) $score = $question['points'];

        $db->execute_query("INSERT INTO submissions (exam_id, student_id, question_id, answer, score) VALUES ('$exam_id', " . $student['id'] . ", " . $question['id'] . ", '" . $answer . "', " . $score . ") ");
    }
    header("location: /");
}
$questions_shuffled = [];
while ($question = $questions2->fetch_assoc()) $questions_shuffled[] = $question;
...
<body>
    ...
    <div class="flex gap-10 p-10">
        ...
        <div class="flex-1">
            ...
            <?php } else { ?>
                <div <?= $exam['is_rtl'] == 0 ? 'dir="ltr" ' : 'dir="rtl" ' ?>class="bg-white border p-10">
                        ...
                        foreach ($questions_shuffled as $c => $question) { ?>
                            <div class="py-2">
                                ...
                                <?php } else if ($question['type'] == 'short_answer') { ?>
                                    ...
                                    $left = $db->query("SELECT * FROM matching_pairs WHERE parent_id = 0 AND question_id = " . $question['id'] . " ORDER BY rand()")->fetch_all(MYSQLI_ASSOC);
                                    $right = $db->query("SELECT * FROM matching_pairs WHERE parent_id > 0 AND question_id = " . $question['id'] . " ORDER BY rand()")->fetch_all(MYSQLI_ASSOC);
                                    foreach ($right as $right_ele) { ?> 
                                        <div class="mb-2 flex items-center gap-2">
                                            <div><?= $right_ele['text'] ?>: </div>
                                            <select name="right_<?= $right_ele['id'] ?>" class="border rounded bg-gray-100 py-1 px-2">
                                                <option disabled selected>---</option>
                                                <?php foreach ($left as $left_ele) { ?>
